...
final class AgeViewModel {
    ...
    fileprivate typealias AgeResult = Result<String, AgeValidationError>
    
    // MARK: - VC에서 들어온 데이터
    // 처음 들어올 때 빈값에 대한 에러를 리턴해서
    // 옵셔널 값을 주어서 nil에 대한 핸들링을 따로 하게 만들었음
    // lazyBind를 따로 만들어 closure의 호출 시점을 init할 때가 아닌 값이 변할때만 하도록 함
    var inputAgeText = Observable("")
    
    // MARK: - View에서 바라보는 데이터
    // Result를 처리해서 String으로 주는 것까지가 뷰모델의 역할일것 같음..
    private(set) var outputText = Observable("")
    ...
    init() {
        ...
        // 해결방법 1. 옵셔널 처리를 통한 에러처리
        // 해결방법 2. lazyBind 생성
        inputAgeText.lazyBind { [weak self] _ in
...
private extension AgeViewModel {
    func validationCheckLabel() {
        let result = validateAgeInput(value: inputAgeText.value)
    ...
    // 파라미터 사용 -> 유지보수
    // 외부종속성을 배제하는 방법 중 하나 (원본에 대한 접근)
    func validateAgeInput(value: String) -> AgeResult {
...
fileprivate extension AgeViewModel {
