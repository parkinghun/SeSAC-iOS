...
extension BaseballGameView {
    ...
    private func validateInput(_ input: String) -> (isValid: Bool, message: String) {
        ...
        if Set(input).count != 3 {
            return (false, "중복되지 않은 3자리 숫자를 입력해주세요.")
        }
    ...
    private func checkGuess() {
        ...
        let (strikes, balls) = matchNumbsers(input: userInput)
    ...
    private func matchNumbsers(input: String) -> (strikes: Int, balls: Int){
        let inputArr = input.compactMap { Int(String($0)) }
        
        let strike = zip(secretNumber, inputArr).filter(==).count
        let ball = Set(secretNumber).intersection(inputArr).count - strike
